...
| I | 直近の出席日 | **数式**（マクロではない）。出席シートから同氏名の日付を新しい順に最大 10 件表示 |
| J | カードID | NTAG215 の UID（例: `0440B2AA852190`） |

「直近の出席日」は出席シートを参照するシート数式です。  
アプリは日付・時刻を **本物の日付/時刻セル**として書くため、この数式が正しく更新されます  
（文字列のまま書くと数式が空や `, ,` になることがあります）。
...
打刻のたびに 1 行追加されます。Web の「打刻履歴」もこのシートを読みます。
